/**
 * Grade a prop pick against a box score row (used by API + track record).
 */

function propActualValue(log, propType) {
  if (!log) return null;
  const type = String(propType || '').toLowerCase();
  if (type === 'pra') {
    const pts = Number(log.pts);
    const reb = Number(log.reb);
    const ast = Number(log.ast);
    if (![pts, reb, ast].every(Number.isFinite)) return null;
    return pts + reb + ast;
  }
  const value = Number(log[type]);
  return Number.isFinite(value) ? value : null;
}

function gradePropPick(pick, log, game) {
  const status = String(game?.status || '').toLowerCase();
  const isFinal = status === 'final' || status === 'closed' || status === 'complete';
  const actualValue = propActualValue(log, pick.prop_type);
  const line = Number(pick.line);
  const recommendation = String(pick.recommendation || '').toUpperCase();

  if (!Number.isFinite(line) || !['OVER', 'UNDER'].includes(recommendation)) {
    return { actual_value: actualValue, result: null, result_label: null, hit: null, dnp: false };
  }

  if (!isFinal) {
    return { actual_value: actualValue, result: null, result_label: null, hit: null, dnp: false };
  }

  // Game is final but no log found — player did not play (DNP). Grade as 0.
  const isDnp = actualValue == null && log == null;
  const gradedValue = isDnp ? 0 : actualValue;

  if (gradedValue == null) {
    return { actual_value: null, result: null, result_label: null, hit: null, dnp: false };
  }

  if (gradedValue === line) {
    return { actual_value: gradedValue, result: 'push', result_label: 'PUSH', hit: null, dnp: isDnp };
  }

  const hit = recommendation === 'OVER'
    ? gradedValue > line
    : gradedValue < line;
  return {
    actual_value: gradedValue,
    result: hit ? 'hit' : 'miss',
    result_label: hit ? 'HIT' : 'MISS',
    hit,
    dnp: isDnp,
  };
}

module.exports = { propActualValue, gradePropPick };
